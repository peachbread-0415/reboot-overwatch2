...
- **인스타그램**
    ...
    - 콘텐츠 포맷별 세부 전략
        - ① 루틴형 콘텐츠
            - 위클리 프로 Tip
                - [상황 발생] → [위치/에임 포인트 확대 스크린샷] → [성공 예시]로 구성된 3~4컷 카드뉴스
                - 화제가 되었던 장면을 정지 이미지로 정확하게 짚어주는 공
                - 카드뉴스 마지막 장 :
                  이 기술이 결정적으로 쓰인 경기 장면은? ☞ @OW_Esports 을 넣어서
                  랜딩페이지로 자연스럽게 이스포츠 계정으로 연결되게끔 한다.
            - 알.쓸.오.잡 (알아두면 쓸데있는 오버워치 잡학사전)
                - 'C9', '스태거', '포킹' 등 이스포츠 해설 용어/은어 뜻풀이 카드뉴스
                - 초보 유저들이 중계를 볼 때 느끼는 진입장벽(용어)을 해소하고 더 깊이 있는 관람을 돕는 가이드
                - 카드뉴스 마지막 장 :
                  오늘 배운 'C9' 실수가 나온 안타까운 경기는? ☞ @OW_Esports 을 넣어서
                  랜딩페이지로 자연스럽게 이스포츠 계정으로 연결되게끔 한다.
            - 위클리 한타 전략
                - 맵을 위에서 본 탑뷰 이미지에 영웅 이동 경로와 진형을 화살표로 도식화한 전술 분석
                - 화려한 스킬 이펙트에 가려 영상으로는 파악하기 힘든 팀의 진영 싸움과 포지셔닝을 한눈에 정리
                - 카드뉴스 마지막 장 :
                  이 완벽한 포위망이 어떻게 작동했는지 영상으로 확인하기 ☞ @OW_Esports 을 넣어서
                  랜딩페이지로 자연스럽게 이스포츠 계정으로 연결되게끔 한다.                  
            - 개발진 Q&A 카드뉴스(격주)
                - 밸런스, 매칭, 신규 모드 관련 질문을 정리해 카드뉴스 형태로 발행
                - '우리가 유저 이야기를 듣고 있다'는 메시지를 시각적으로 전달 및
                  굳이 업데이트 게시판을 보지 않아도 짧게 내용을 요약해주는 역할
...
- 핵심 목표
    - 정기 업로드 루틴 확립 → 지속 노출 → 스토어(steam)유입 및 즉각 다운로드 전환
...
- 정량적 효과
    - 게시물 및 프로필 링크에 딥링크(steam://store/2357570)를 적용하여 웹페이지 이동 → 로그인 과정을 생략하고 클릭 즉시 스팀 설치 창 팝업 유도
    - 인스타·유튜브 콘텐츠 소비 유저의 실제 게임 접속 전환율 7~10% 달성


#### 2. 글로벌 행사(G-STAR 2025) 기반 UA 전략

- 핵심 목표
    - 오프라인 체험 → QR 코드(스팀 딥링크) 스캔 → 현장 즉시 라이브러리 추가
    - 신규·복귀 유저 전환, 브랜드 신뢰 회복
    - 사회공헌형 굿즈로 브랜드 가치 제고
- 정성적 효과
    - 체험존·시연존 중심의 현장 몰입도 향상 → 함께 플레이 감정 회복
    - Vision Collection 기부(약 2,500만 원) 으로 사회적 브랜드 이미지 상
- 정량적 효과
    - 부스 곳곳에 스팀 딥링크 QR을 배치하여 유저가 현장에서 즉시 게임을 라이브러리에 추가하도록 유도 (전환율 25% 목표)
    - 행사 기간 스팀 플랫폼 노출을 통해 스 미설치 휴면 유저의 재설치 비율 15% 이상 달성
    - SNS 확산량 +500%, 전체 부스 방문객 대비 신규 설치 전환율 10~15% 달성
